...
# Load the data
data = pd.read_csv("app\static\water_potability.csv")
data = data.dropna()
...
def index():
    ...
    return render_template('index.html', static_plot=static_plot,dropdown_options=dropdown_options, histogram_plot=histogram_plot,
                           selected_feature=selected_feature, accuracy=accuracy, precision=precision, recall=recall, f1=f1, conf_matrix=conf_matrix_plot)
...
def preprocess_data(data):
    ...
    column_order = ['ph', 'Hardness', 'Solids', 'Chloramines', 'Sulfate', 'Conductivity', 'Organic_carbon', 'Trihalomethanes', 'Turbidity']
    ...
    numerical_features = ['Conductivity', 'ph', 'Turbidity']  # Only scale these features
...
def predict():
    ...
    prediction_label = "Potable" if prediction[0] == 0 else "Not Potable"
